...
namespace Editor.Util
{
    public static class ReflectionHelper
    {
        ...
        private readonly static Lazy<Dictionary<DungeonObjectKind, Type>> _cacheDungeonPlacers = new(() =>
        {
            var candidates = AppDomain.CurrentDomain.GetAssemblies()
                .SelectMany(s => s.GetTypes())
                .Where(p => Attribute.IsDefined(p, typeof(PlacerOfAttribute)));

            return candidates.ToDictionary(p => ((PlacerOfAttribute)p.GetCustomAttribute(typeof(PlacerOfAttribute))!).Kind, p => p);
        });

        /// <summary>
        /// Retrieve the dungeon placer for a particular dungeon object kind.
        /// </summary>
        public static Type GetDungeonPlacerFor(DungeonObjectKind kind)
        {
            if (_cacheDungeonPlacers.Value.TryGetValue(kind, out Type? t))
            {
                return t;
            }

            return typeof(DungeonAssetPlacer);
        }
    }
}
