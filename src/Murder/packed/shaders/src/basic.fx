...
float4 DiagonalLinesPixelShader(VSOutput input) : SV_Target0
{
	...
	float4 color = tex2D(inputTexture, input.texCoord0);
	return
		input.texCoord1.x * color * float4(input.color.rgb, input.color.a * fxAlpha) +						 // Basic		  //
		input.texCoord1.y * float4(lerp(color.rgb, input.color.rgb, input.color.a), color.a * fxAlpha) +	//  Wash		 //
		input.texCoord1.z * float4(input.color.rgb, input.color.a * fxAlpha);							   //   Color onlly //
}
