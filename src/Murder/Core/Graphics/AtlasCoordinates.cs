...
namespace Murder.Core.Graphics
{
    ...
    public readonly struct AtlasCoordinates
    {
        ...
        /// </summary>
        public readonly Point Size { get; init; }
        public readonly IntRectangle SourceRectangle;
        public readonly Rectangle UV;
        /// <summary>
        /// The offset of the bounding box of the pixels that survived trimming
        /// </summary>
        ...
        {
            (Name, SourceRectangle, ContentOffset, AtlasIndex) = (name, atlasRectangle, contentOffset, atlasIndex);
            AtlasId = atlasId;
            Size = size;
        ...
        public void Draw(Batch2D spriteBatch, Vector2 position, Rectangle clip, Color color, Vector2 scale, float rotation, Vector2 offset, ImageFlip imageFlip, Vector3 blend, MurderBlendState blendState, float sort)
        {
            ...
            if (clip.IsEmpty)
            {
                ...
                Vector2 finalOffset = new Vector2(
                        flipH ? (SourceRectangle.Width + ContentOffset.X) + offset.X : offset.X - ContentOffset.X,
                        flipV ? (SourceRectangle.Height + ContentOffset.Y) + offset.Y : offset.Y - ContentOffset.Y);
            ...
            else
            {
                ...
                IntRectangle intersection = IntRectangle.FromAbsolute(
                    Math.Max(clip.Left, ContentOffset.X),
                    Math.Max(clip.Top, ContentOffset.Y),
                    Math.Min(clip.Right, ContentOffset.X + SourceRectangle.Width),
                    Math.Min(clip.Bottom, ContentOffset.Y + SourceRectangle.Height));
