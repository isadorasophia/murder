...
public static partial class Ease
{
    ...
    /// <summary>
    /// ASR (Attack-Sustain-Release) envelope. <paramref name="attackRatio"/> is the time to reach the peak,
    /// <paramref name="sustainRatio"/> is the time it stays on the peak, and the rest is the release time.
    /// </summary>
    /// <param name="t">Time elapsed (0 to 1).</param>
    /// <param name="attackRatio">Ratio of time for attack phase.</param>
    /// <param name="sustainRatio">Ratio of time to hold at peak.</param>
    /// <param name="attackEase">Custom easing curve for attack phase.</param>
    /// <param name="releaseEase">Custom easing curve for release phase.</param>
    /// <returns>Envelope value (0 to 1 to 0).</returns>
    public static float Envelope(float t, float attackRatio = 0.1f, float sustainRatio = 0f, EaseKind attackEase = EaseKind.CubeIn, EaseKind releaseEase = EaseKind.CubeOut)
    {
        float releaseRatio = 1f - attackRatio - sustainRatio;

        if (t <= attackRatio)
        {
            // Attack phase: 0 to 1
            float attackT = t / attackRatio;
            return Evaluate(attackT, attackEase);
        }
        else if (t <= attackRatio + sustainRatio)
        {
        ...
        else
        {
            ...
            float releaseT = (t - attackRatio - sustainRatio) / releaseRatio;
            return 1f - Evaluate(releaseT, releaseEase);
        }
    }
}